...
class TemplateModelBase(BaseModel):
    template_str: TemplateStrType = Field(examples=['str'])
    template_int: TemplateIntType = Field(examples=[1])
    template_UUID4: TemplateUUID4Type = Field(examples=['f310e0ab-e8ba-43a0-8c50-12c95827bc21'])
    template_float: TemplateFloatType = Field(examples=[3.14])
    template_str_enum: TemplateStrEnumType = Field(examples=[TemplateStrEnumType.ONE.value])
    template_int_enum: TemplateIntEnumType = Field(examples=[TemplateIntEnumType.ONE.value])
    template_enum: TemplateEnumType = Field(examples=[TemplateEnumType.FIRST.value])
    template_literal: TemplateLiteralType = Field(examples=['a'])
    template_string_constraints_type: TemplateStringConstraintsType = Field(examples=[' A '])
